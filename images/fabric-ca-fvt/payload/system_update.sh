...
apt-get -y install --no-install-recommends rsyslog bc vim lsof sqlite3 haproxy postgresql-$PGVER \
           ...
           debconf-utils zsh htop python3-minimal libpython3-stdlib \
           parallel netcat-traditional wget lsb-release gnupg ca-certificates || let RC+=1
...
# Use python2, not 3
ln -s /usr/bin/python3 /usr/local/bin/python && chmod 777 /usr/local/bin/python || let RC+=1
